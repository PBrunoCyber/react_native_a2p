...
interface IProps {
    ...
    formErrors: any,
    context: IQuantidadeEquipamentos
}

const QuantidadeDeEquipamentos = ({ quantidadeDeEquipamentos, answerEquipamentosAlunosInternet, formErrors, context }: IProps) => {
    const [isClicked, setIsClicked] = useState(false);
    const [answer, setAnswers] = useState<IQuantidadeEquipamentos>(context);

    useEffect(() => {
        quantidadeDeEquipamentos(answer);
        for (const key in answer) {
            if (answer[key as keyof IQuantidadeEquipamentos]) {
                setIsClicked(true);
            }
        }
    }, [answer])
