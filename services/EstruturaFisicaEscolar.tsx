...
const deleteEstruturaFisicaEscolar = (inep: string) => {
    return new Promise ((resolve, reject)=> {
        db.transaction((tx)=> {
            tx.executeSql("DELETE FROM tb_estrutura_escolar WHERE inep_fk = ?",
            [inep],
            (tx,results)=> {
                if(results.rowsAffected > 0){
                    resolve(true);
